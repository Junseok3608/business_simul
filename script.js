...
async function loadExternalSejongData() {
    try {
        ...
        console.log("외부 데이터 파이프라인 연동 통로 정상 작동 중:", externalPragmaticCriteria.source);
    } catch (error) {
...
function startLoading() {
    nextPage(7);
    const messages = [
        "입력하신 사회언어학적 가감 변수와 직무 과업을 스캐닝하고 있습니다...",
...
당신은 한국 대기업에서 10년 이상 근무하며 수많은 외국인 직원과 유학생들의 직무 지도를 담당해 온 조직 관리 전문가이자 비즈니스 한국어 화용론 코치입니다(Deshpande et al., 2023 적용). 당신은 철저히 중립적이고 전문적인 어조를 유지하며, 학습자의 문법적 오류는 교정 대상에서 제외하고, 오직 한국의 사내 조직 문화와 위계질서에 따른 '사회언어학적 화용적 적절성'만을 정밀하게 평가해야 합니다.
...
[Evolve]
학습자의 대화 초안이 화용론적 결례로 인해 감점을 받았을 경우, 단번에 정답 문장을 완벽하게 제공하여 상황극을 종료하지 마십시오(Madaan et al., 2023 적용). 해당 맥락의 정중성 임계치를 통과할 수 있는 '어휘적 에어백(힌트)'만을 단계적으로 제공하며 발화를 반려하고, 학습자가 스스로 문장을 수정(Iterative Refinement)해 오도록 유도하십시오.`;
